...
Changes 01–07 + `admin-local-identity` arquivados. Change 08 `scheduler-multi-unit` **executado** (2026-09-08; 3 slices; gate 806 testes ×2 rodadas + ruff/format/mypy + `openspec validate --strict` PASS; aguardando autorização para arquivar). Estágio do agendamento completo (plano §4/§10-08): `Case` ganhou `SchedulingUnit` 1|2 + 7 campos de agendamento (migration única 0008); `apps/scheduler`: serviços transacionais `confirm_case_scheduling`/`deny_case_scheduling` (**branch por estado**: `await_scheduling_confirmation` só de SCHEDULER_REQUESTED; de AWAITING_SCHEDULING pula o await — 3 ou 2 eventos no mesmo atomic) com **resposta final ao NIR na thread** por unidade (unidade 2 = texto EXATO do plano §4 sem ponto final) e **transição NOVA `reopen_scheduling`** (FINAL_REPLY_POSTED → AWAITING_SCHEDULING; 2ª transição nova pós-change-03, prevista no guardrail D4/03) + `reopen_scheduling_after_incident` (**intercorrência só unidade 1**, erro nomeado na 2; limpa os 5 campos `scheduled_*`, motivo obrigatório, ciclo reabrir→reconfirmar testado); UI: fila completa sem filtro por unidade (abas aguardando = SCHEDULER_REQUESTED|AWAITING_SCHEDULING FIFO + processados, Paginator, guards por papel ativo scheduler/admin), **detalhe limitado ao necessário alinhado ao ats-web (decisão do dono 2026-09-08)** — identificação real + **one-liner re-identificado só na renderização** (`reidentify_text` na 1ª linha do `summary_text`; zero artefatos clínicos/policy/estrutura na página) + decisões médicas + agendamento + thread, forms confirmar/negar/desmarcar (erros → mensagem+redirect, nunca 500), **PDF do relatório por `position`** (`CaseDocument` multi-PDF — `case.pdf_file` do ats-web não existe no HMD; desvio escalado e resolvido via supervisor: rota espelhando `doctor:case_pdf`, gate `scheduled_by==user` + status pós-decisão, 404 fail-closed p/ outro agendador/caso reaberto, links de todos os docs só nessa condição). Invariante preservada: re-identificação SÓ na renderização; LLM segue vendo apenas tokens.

Notas operacionais: portas host 5432/5433/55433 flutuam entre projetos — use `POSTGRES_HOST_PORT=55432`/`TEST_DB_PORT=55435`. Produção exige NTP + cache compartilhado + workers no ar (flags `*_RUN_TASKS_INLINE=false`); LLM em prod exige `OPENROUTER_API_KEY` + `LLM_MODEL_*` (modelos por env). Diagnósticos: `manage.py ad_check` (AD), `manage.py llm_check` (OpenRouter), `manage.py anonymization_benchmark --corpus <real>` (**aceite pré-produção: relatórios reais e revisão de recall por entidade**), `manage.py seed_prompts` (idempotente). Pendências de validação em ambiente: qcluster real (worker-pdf/worker-anonymization/worker-llm) e `docker build` das imagens de worker (~700 MB com modelo); **benchmark de modelos flash + corpus real de anonimização pré-produção**; P2s do 06: `released=True` só após commit do atomic (anonymization/orchestrador), branch async de conflito sem teste direto, revisão humana dos 28 prompts; **K textual-informativo pendente de validação do dono**. P2s do admin-local: replicação do form no pre-check do /admin (sincronizar em upgrades do Django), `_wrapped` privado no teste de wiring, ADR-0004 fica histórico (0009 o emenda). P2s do 07: teste do ramo fail-closed do access, assertNumQueries da fila, card de decisões gateado por decision_event (rows são a fonte), re-render de POST inválido em corrida perdedora. P2s do 08: `scheduling_reopen_reason` persiste pós-reconfirmação e segue no card de agendamento (histórico vive nos eventos), mensagens default EN nos required do form de confirmação, duplicação de helpers de teste entre test_services/test_incident, teste de atomicidade simula corrida via validação concorrente. Quirk de harness documentado: flush de TransactionTestCase + --reuse-db apaga seeds de migration (mitigado com teste chamando a função da migration + fixtures get_or_create). Próximo: change 09 `nir-result-closure` (resultado final, ack NIR → CLEANING → CLEANED; DOCTOR_DENIED só anda aqui).
