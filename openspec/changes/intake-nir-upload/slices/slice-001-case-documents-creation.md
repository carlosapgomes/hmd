...
- Referência (somente-leitura): `/projects/dev/ats-web/apps/intake/{services,forms,views}.py` (padrões de validação de upload e form de intake — o ats-web é single-PDF: `Case.pdf_file`; HMD diverge para multi, ver design D1). Validação de arquivo do ats-web: `validate_attachment_file` em `apps/intake/services.py` (PDF-only, content-type).
