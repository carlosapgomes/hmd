...
- **R3** Blocos específicos por tipo (13): pelo menos os três nomeados no plano + um campo específico por tipo derivado da sua seção clínica (ex.: angio_carotidas → sintoma neurológico recente; dren_biliar → colestase/ICG quando informado); tabela tipo→bloco documentada no módulo.
- **R4** `build_llm1_schema(types: Sequence[str]) -> type[BaseModel]`: dinâmico com base + união dos blocos (fields mesclados; tipos únicos validados contra o catálogo; duplicata de tipo → erro); usado por LLM1 (uma chamada por caso) e reutilizável.
- **R5** `Llm2Response`: `summary_text` + `procedures[]` (tipo + sugestão `aceitar|recusar` + motivos[] + evidence) — valida a saída da sumarização.
- **R6** `normalize_schema_for_response_format(model) -> dict`: gera JSON schema pydantic com `oneOf→anyOf` (compatibilidade OpenRouter/OpenAI structured outputs).
...
| R6 | `apps/pipeline/schemas/adapters.py` | `::test_oneof_to_anyof` |
