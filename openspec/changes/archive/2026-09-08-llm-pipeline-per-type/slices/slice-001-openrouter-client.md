...
- **R3** `OpenRouterClient` (ou factory `get_llm_client()`): usa SDK OpenAI com `base_url=OPENROUTER_BASE_URL` (default `https://openrouter.ai/api/v1`), `api_key=OPENROUTER_API_KEY`, `timeout=LLM_TIMEOUT_SECONDS` (default 120); método `complete(model, messages, *, json_schema=None) -> str`: com `json_schema`, envolve no formato strict do endpoint (`{"type": "json_schema", "json_schema": {"name", "schema", "strict": true}}` — padrão ats-web); devolve o conteúdo; mapeia erros da SDK para `LlmError` com kind correto (401/403→auth; 429→rate_limit; `APITimeoutError`→timeout; `APIConnectionError`→network; demais→other). Cliente novo por chamada ou reuso seguro — documentar.
