...
- **R1** `pyproject.toml` + lock: `django-q2` pinado; `config/settings/base.py`: `INSTALLED_APPS` com `django_q` + `Q_CLUSTER` no formato do ats-web — **`ALT_CLUSTERS` DENTRO de `Q_CLUSTER`**: `{"name": "hmd", "orm": "default", "ALT_CLUSTERS": {"pdf": {"workers": 2, "timeout": 180}}}`; migrations do django_q aplicáveis em banco limpo; `test.py` sem qcluster real (tasks chamadas direto).
