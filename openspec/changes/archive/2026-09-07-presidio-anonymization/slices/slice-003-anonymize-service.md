...
- **R1** `PseudonymOperator` — componente próprio de mapeamento de tokens (emenda pós-spike, design D5: a API de operador custom da 2.2.364 **não sustenta** o padrão — chave `"lambda"`, callable sem entity_type, ordenação decrescente; **sem herança de `Operator`**, o `AnonymizerEngine` não faz a substituição): instância por chamada; devolve token da categoria (`PESSOA/CPF/CNS/CRM/DATA/LOCAL/ORGANIZACAO/TELEFONE/EMAIL/OCORRENCIA/PII`) numerado por primeira ocorrência; dedupe por **chave canônica por categoria** (variantes de caixa/formato = mesmo token; mapa guarda a forma original da primeira ocorrência); expõe `mapping` (token → {value, entity_type}); tokens `<CATEGORIA_N>`; a substituição sobre os spans mesclados ordenados por offset é do serviço.
- **R2** Merge determinístico×NLP em `apps/anonymization/services.py`, com **política determinística completa** (design D6): localizar TODAS as ocorrências de cada valor determinístico no texto (offsets válidos); categorizar (nome→PESSOA, nascimento→DATA, **nº de ocorrência→OCORRENCIA**, CPF→CPF, CNS→CNS); ordenar todos os spans por `(start asc, end desc, determinístico > NER)`; sobreposições resolvem mantendo o primeiro da ordem; testes para cada caso de sobreposição (mesma categoria, categorias diferentes, contenção parcial, empate).
- **R3** Regra de escopo: TODAS as entidades do escopo (incluindo nome/nascimento/CPF/CNS determinísticos e o nº de ocorrência quando presente no corpo, com token `<OCORRENCIA_N>`) são substituídas — o corpo anonimizado não contém nenhum identificador; o linkage usa os campos persistidos (D2), não o texto.
- **R4** **Núcleo puro `anonymize_text(text) -> AnonymizationCoreResult`** (passos 1–5 do design D6, SEM DB — consumido pelo benchmark) + **wrapper `anonymize_case_text(case)`** que, numa transação, grava em `case`: `anonymized_text`, `pseudonym_map` (JSON), `anonymization_report` (JSON: contagens por tipo, `ANONYMIZATION_SPACY_MODEL`, versões presidio, threshold), `patient_name`, `patient_birth_date` (e `agency_record_number` se determinístico achou e ainda vazio) + `CaseEvent` `CASE_ANONYMIZATION_COMPLETED` (payload: contagens + versões). **A task (slice 004) envolve wrapper + `complete_anonymization` num único `transaction.atomic()`** — este slice NÃO faz transição de estado, e o porteiro do texto vazio é a task (aqui o núcleo é defensivo com texto vazio).
