...
- **R3** Task `process_case_attachments(case_id)` no cluster `attachments`
  ...
  `ATTACHMENTS_RUN_TASKS_INLINE` default true): idempotente (pula anexos em
  estado terminal); para cada anexo `pending`: `processing` → extração →
  persiste `extracted_text`/`extraction_method` (status permanece
...
- **R6** Testes: extração local (PDF com texto; SEM evento externo);
  ...
  intactos; teto de páginas; task idempotente (segunda execução não
  reprocessa terminal); trigger dispara no `CASE_ANONYMIZATION_COMPLETED`
  via on_commit (precedente de teste: `apps/pipeline/tests/test_signals.py`
  com `django_db(transaction=True)`); inline flag respeitada
  (`override_settings(ATTACHMENTS_RUN_TASKS_INLINE=False)` + assert de
  enqueue). Vision/LLM sempre com fakes injetáveis (monkeypatch) — suíte
  sem rede.
...
| R3 | `apps/attachments/tasks.py` | `test_task_processes_pending`, `test_task_idempotent`, `test_task_vision_failure_marks_failed_other_attachments_intact` |
