...
- **R1** `reidentify_text(case, text) -> str`: substitui tokens pelos valores do `pseudonym_map`, **ordenados por comprimento de token decrescente** (evita `<PESSOA_1>` casar dentro de `<PESSOA_10>`); mapa vazio → texto inalterado; função pura sobre (mapa, texto).
- **R2** `manage.py anonymization_benchmark --corpus <path> [--min-recall X]`: lê JSONL `{"text": ..., "expected": [{"value": ..., "entity_type": ...}]}`; para cada entrada roda o pipeline de anonimização (serviço do 003 sobre texto puro — sem caso/DB), mede: acerto por entidade esperada (valor ausente do output), contagens por tipo detectado, latência por documento; agrega recall por `entity_type`, p50/p95, entidades totais; **varredura zero-PII** (regex CPF/CNS com checksum no output = falha); imprime relatório; exit 0 somente se todo tipo ≥ `--min-recall` (default `ANONYMIZATION_BENCHMARK_MIN_RECALL`, 0.90) E zero-PII limpo.
...
| R2 | `apps/anonymization/management/commands/anonymization_benchmark.py` | `test_benchmark.py::test_synthetic_corpus_passes`, `::test_adversarial_corpus_fails`, `::test_zero_pii_violation_fails` |
