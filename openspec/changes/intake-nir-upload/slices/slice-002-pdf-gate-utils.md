...
- **R4** `evaluate_regulation_report(text) -> GateResult` congelável (dataclass frozen com `ok`, `reason_code`, `matched_institutional_signals`, `matched_operational_sections`, `text_length` — diagnóstico alimenta o motivo exibido ao NIR). **Listas canônicas HMD** (design D4; normalização sem acentos/case-insensitive/whitespace colapsado): header "RELATÓRIO DE OCORRÊNCIAS"; institucionais "Central Estadual de Regulação"/"Secretaria da Saúde do Estado"/"Governo do Estado da Bahia"; seções "Código:"/"Abertura:"/"Unid. Origem"/"Unidade de Origem"/"Motivo da Solicitação"/"Complemento da Solicitação"/"Resumo Clínico"/"Dias em tela"/"Data Adm. Unid.". Thresholds: nº de seções ≥ `INTAKE_REGULATION_MIN_OPERATIONAL_SECTIONS`, `len ≥ INTAKE_REGULATION_MIN_TEXT_CHARS`; `reason_code` canônico por falha ("missing_header", "below_min_chars", "insufficient_sections", "missing_institutional_signals").
...
- **R5b** Teste de ordem combinada: nº de ocorrência que aparece TAMBÉM como marca d'água repetida → extraído do TEXTO BRUTO antes do strip (o texto limpo não o contém mais, mas o nº foi preservado).
