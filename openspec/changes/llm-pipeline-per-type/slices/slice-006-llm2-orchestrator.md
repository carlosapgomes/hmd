...
- **R1** `run_llm2_summarization(case, *, user=None, role="system")`: tipos = **reconciliados** (declarados — bypass preserva declarado); monta `visao_estruturada` = cópia efêmera do `structured_data` filtrada pelos tipos (padrão ats-web) + `policy_result` + prior-case summaries; **assert de tokens** (nenhum valor do `pseudonym_map` no payload); prompts `llm2` (003); 1 chamada; validação strict (schema Llm2 do 002) + language guard + retry único (regime do 004); falha → `fail_processing("llm2_<motivo>")`. Sucesso: **reconciliação final** — por procedimento, `policy.recomendacao == recusar → sugestão = recusar(motivos_policy)` (LLM não suaviza); agregado do caso = mais restritiva (strictest); persiste `Case.summary_text` + `Case.suggested_action` + evento `CASE_LLM2_COMPLETED` (resumo: sugestões por tipo, versions).
- **R2** `Case`: +`summary_text TextField blank` + `suggested_action JSONField default dict` (migration sincronizada com 004/005 conforme lá).
...
- **R5** Signals (apps/pipeline/signals.py + AppsConfig.ready): `CASE_STATUS_LLM_EXTRACTING` com `source=="ANONYMIZING"` → enqueue; `CASE_STATUS_LLM_SUMMARIZING` com `source=="LLM_EXTRACTING"` → enqueue (retomada é acionada pelo próprio orquestrador que já está rodando — na prática o signal de SUMMARIZING cobre o **bypass** que vem do intake, `source` do bypass é `LLM_EXTRACTING` ✓ e a transição natural do pipeline NÃO re-dispara pois seu source também é LLM_EXTRACTING… **correção**: a transição natural `complete_llm_extraction` TEM source LLM_EXTRACTING e dispararia enqueue duplicado durante a própria task — idempotência por estado+lock resolve (2ª execução = no-op/conflict-no-op async), mas prefira evitar: o enqueue de retomada só quando `source=="LLM_EXTRACTING"` **E** o evento não veio da própria task → distinguir por `actor_type=="user"` (bypass é do NIR; avanço natural é system). Documentar e testar os dois casos.) `transaction.on_commit` em ambos.
...
| R5 | `apps/pipeline/{signals,apps}.py` | `test_signals.py::test_entry_enqueues`, `::test_natural_advance_no_duplicate`, `::test_bypass_enqueues_resume` |
...
expected_files:
  ...
  - apps/cases/events.py            # +CASE_LLM2_COMPLETED (se não no 004)
