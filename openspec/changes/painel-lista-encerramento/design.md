...
- FSM no MODELO (padrão do repo): hook privado
  ...
  que chama `_run_transition` e grava o evento
  ...
  Nenhum service chama `_run_transition` direto.
- **No atomic do service, nesta ordem**: (1) snapshot dos campos de lock
  para o payload; (2) `force_release_case_lock(case, *,
  reason="administrative_closure")` — função pública NOVA em
  `apps/cases/locks.py` que espelha `release_case_lock` (re-lê sob
  `select_for_update` e **muta a mesma instância** — o `save()` posterior
  da transição NÃO ressuscita o lock) e grava `CASE_LOCK_RELEASED` com
  payload de força; (3) coletar `file.name` de documentos/anexos ANTES do
  delete (padrão do ack) e **minimização completa** via
  `_clean_acknowledged_clinical_data` (rows) no mesmo módulo; (4) op pública
  do modelo → `CASE_STATUS_CLEANED` + `CASE_ADMINISTRATIVELY_CLOSED`
...
- **Workers em voo — duas camadas**: (a) recusa por lease viva (acima)
  cobre o caso comum; (b) **abort em status CLEANED antes de cada passo
  mutante** nas 3 tasks (`apps/intake/tasks.py`,
  `apps/anonymization/tasks.py`, `apps/pipeline/orchestrator.py` — os
  `refresh_from_db` já existem) E **validação de status dentro dos atomics
  de escrita clínica** (`apps/pipeline/policy.py`, `llm2_service.py`,
  `prior_case.py`: re-lê o caso dentro do atomic e retorna sem persistir se
  `CLEANED`) — cobre o worker travado na LLM com lease EXPIRADA que
  retorna depois do encerramento e repovoaria `policy_result`/
  `summary_text`/`suggested_action`. Teste pinnado: lease expirada →
  encerra → passo do pipeline roda → campos clínicos seguem zerados.
