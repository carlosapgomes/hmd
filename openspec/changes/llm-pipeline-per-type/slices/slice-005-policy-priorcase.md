...
- **R2** Recomendação global por procedimento: `recomenda_aceitar` (sem alertas de recusa) | `recomenda_recusar(motivos)` — mapeamento determinístico: alertas de **threshold crítico da seção** (definidos por tabela no módulo, derivados de parametrosHMD) somam motivos de recusa; os demais ficam como alertas informativos. **Nunca bloqueia** — é recomendação+explicação (spec).
...
- **R6** Settings/env: `PRIOR_CASE_WINDOW_DAYS` (7), `PRIOR_CASE_FALLBACK_WINDOW_DAYS` (15) + `.env.example`; `Case`: +`policy_result JSONField default dict` (migration se não entrou no 004 — sincronizar).
...
allowed_incidental_files:
  - apps/cases/models.py + migration (apenas SE o campo policy_result não tiver entrado no 004)
out_of_scope:
